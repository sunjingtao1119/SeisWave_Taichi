...
class ElasticWAVE:
    def __init__(self,
        ...
        self.memory_sxx_dx = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_sxx_dx)

        self.memory_sxz_dz = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_sxz_dz)

        self.memory_sxy_dy = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_sxy_dy)

        self.memory_syy_dy = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_syy_dy)

        self.memory_sxy_dx = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_sxy_dx)

        self.memory_syz_dz = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_syz_dz)

        self.memory_szz_dz = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_szz_dz)

        self.memory_sxz_dx = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_sxz_dx)
        
        self.memory_syz_dy = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_syz_dy)


        self.memory_dvx_dx = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvx_dx)
        self.memory_dvx_dy = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvx_dy)
        self.memory_dvx_dz = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvx_dz)

        self.memory_dvy_dx = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvy_dx)
        self.memory_dvy_dy = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvy_dy)
        self.memory_dvy_dz = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvy_dz)
            
        self.memory_dvz_dx = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvz_dx)
        self.memory_dvz_dy = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvz_dy)
        self.memory_dvz_dz = ti.field(fieldtype)
        pixel = ti.root.bitmasked(ti.ijk,self.gridsize)
        pixel.place(self.memory_dvz_dz)
    ...
    # implement Dirichlet boundary conditions on the six edges of the grid
        ...
        for i,j,k in ti.ndrange(nx,ny,(nz-star,nz)):
            ...
            self.vz[i,j,k]=0  
    # source term  
        self.vz[isx,isy,isz]+=dt*Ricker(nt,dt,self.f0,1000)/self.rho[isx,isy,isz] 
       # update sxx szz,syy
