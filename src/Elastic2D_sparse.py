...
class ElasticWAVE:
    def __init__(self,
        ...
        memory=ti.Struct.field(type_dict)
        block = ti.root.bitmasked(ti.ij, self.gridsize)
        block.place(memory)
        self.memory=memory
    ...
    def update_SSG_VS(self,nt:int):   
        ...
        for i,j in ti.ndrange((star+1,nx-star),(star+1,nz-star)):
            ...
            if (x<=xmin_pml or x>=xmax_pml or z<=zmin_pml or z>=zmax_pml):
                self.memory[i,j].sxx_dx = self.b_x[i] * self.memory[i,j].sxx_dx + self.a_x[i] * dsxxdx
                self.memory[i,j].sxz_dz = self.b_z[j] * self.memory[i,j].sxz_dz + self.a_z[j] * dsxzdz
                dsxxdx = dsxxdx/self.k_x[i] + self.memory[i,j].sxx_dx
                dsxzdz = dsxzdz/self.k_z[j] + self.memory[i,j].sxz_dz
        ...
        for i,j in ti.ndrange((star,nx-star-1),(star,nz-star-1)):
            ...
            if (x<=xmin_pml or x>=xmax_pml or z<=zmin_pml or z>=zmax_pml):
                self.memory[i,j].sxz_dx =self.b_x_half[i] * self.memory[i,j].sxz_dx + self.a_x_half[i] * dsxzdx
                self.memory[i,j].szz_dz =self.b_z_half[j] * self.memory[i,j].szz_dz + self.a_z_half[j] * dszzdz
                dsxzdx = dsxzdx /self.k_x_half[i] + self.memory[i,j].sxz_dx
                dszzdz = dszzdz /self.k_z_half[j] + self.memory[i,j].szz_dz
            self.vz[i,j]+=(dszzdz+dsxzdx)*dt/rho_half_x_half_z
       # add source
        self.vz[isx,isz]+=dt*Ricker(nt,dt,self.f0,1) /self.rho[isx,isz] 
       # update sxx szz
        for i,j in ti.ndrange((star,nx-star-1),(star+1,nz-star)):
            ...
            if (x<=xmin_pml or x>=xmax_pml or z<=zmin_pml or z>=zmax_pml):
                self.memory[i,j].dvx_dx= self.b_x_half[i] * self.memory[i,j].dvx_dx + self.a_x_half[i]*dvxdx
                self.memory[i,j].dvz_dz= self.b_z[j]      * self.memory[i,j].dvz_dz + self.a_z[j]     *dvzdz
                dvxdx = dvxdx /self.k_x_half[i] + self.memory[i,j].dvx_dx
                dvzdz = dvzdz /self.k_z[j]      + self.memory[i,j].dvz_dz
    ...
    # update sxz
        for i,j in ti.ndrange((star+1,nx-star),(star,nz-star-1)):
            ...
            if (x<=xmin_pml or x>=xmax_pml or z<=zmin_pml or z>=zmax_pml):
                self.memory[i,j].dvz_dx =self.b_x[i] * self.memory[i,j].dvz_dx+ self.a_x[i] * dvzdx 
                self.memory[i,j].dvx_dz =self.b_z_half[j] * self.memory[i,j].dvx_dz+ self.a_z_half[j]*dvxdz 
                dvzdx =dvzdx /self.k_x[i] + self.memory[i,j].dvz_dx
                dvxdz =dvxdz /self.k_z_half[j] + self.memory[i,j].dvx_dz
